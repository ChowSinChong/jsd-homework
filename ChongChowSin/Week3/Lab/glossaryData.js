...
function onInitialize(items){
  let x = glossary[glossary.length-1].definition
  console.log(x.includes("through"))
...
function getClasses(){
  let dropDownElement = document.querySelector("#classes")
  let classes = [...new Set(glossary.map(item => item.class))]
  // addChild(dropDownElement.parentNode, 'label', 'Class')
  classes.forEach(c => {
    addChild(dropDownElement, 'option', c)
    eventBinding(dropDownElement)
  })
}

function getTags(){
  let filterdiv = document.querySelector('#tags')
  let tags = new Set()
  glossary.forEach(item => tags.add(...[...item.tags]))
  tags.forEach(tag => {
    let inputChild = document.createElement('input')
    inputChild.setAttribute('type','checkbox')
    inputChild.id = tag
    inputChild.value = tag
    eventBinding(inputChild)
    filterdiv.appendChild(inputChild)
...
function eventBinding(element, action){
  element.addEventListener("change", search);
}

function addHeader(parent, item){

}

function addChild(parent, child, value){
...
function refreshContent(items){
  ...
  body.innerHTML = ''
  items.forEach(item => {
      addChild(body, 'p', `${item.term} | ${item.class} | ${item.definition} | ${item.tags}`)
  });
}

function filterItems(items, property, keyword){
  return items.filter(item => item[property].includes(keyword))
}
...
function getFilterredContent(byProperty, data, keyword){
  switch(byProperty.toLowerCase()){
    case definitionConst:
      return data.filter(item => item[definitionConst].includes(keyword))
    case classConst:
      return data.filter(item => item[classConst] == keyword)
    case tagsConst:
      return data.filter(item => item[tagsConst].includes(keyword))
  }
}
...
function search(){
  ...
  let classFilter = document.querySelector("#classes").value
  let filterData = getFilterredContent(classConst, glossary, classFilter)
  
  let tagsFilterElemenets = document.querySelectorAll("#tags input:checked")
  tagsFilterElemenets.forEach(element => filterData = getFilterredContent(tagsConst, filterData, element.value))

  let keyword = document.querySelector("#search").value
  filterData = getFilterredContent(definitionConst, filterData, keyword)
  // let filterred = glossary.filter(item => item.definition.includes(keyword))
